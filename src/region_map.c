...
static const u8 sMapHealLocations[][3] =
{
    ...
    [MAPSEC_BISMARKIA_TOWN] = {MAP_GROUP(BISMARKIA_TOWN), MAP_NUM(BISMARKIA_TOWN), HEAL_LOCATION_BISMARKIA_TOWN},
...
static u8 GetMapsecType(u16 mapSecId)
{
    ...
    case MAPSEC_BISMARKIA_TOWN:
        return FlagGet(FLAG_VISITED_BISMARKIA_TOWN) ? MAPSECTYPE_CITY_CANFLY : MAPSECTYPE_CITY_CANTFLY;
    default:
...
static void CreateFlyDestIcons(void)
{
    ...
    for (mapSecId = MAPSEC_LITTLEROOT_TOWN; mapSecId <= MAPSEC_BISMARKIA_TOWN; mapSecId++)
    {
        if (mapSecId == MAPSEC_BISMARKIA_TOWN){
            canFlyFlag += 2;
        }
